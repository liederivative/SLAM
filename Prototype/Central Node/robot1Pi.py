import random
import numpy as np
from sample_velocity import sampling
class robot:
    def __init__(self):
    ...
    def set_noise(self, new_f_noise, new_t_noise,new_d_noise,\
        ...
        dt = 0.1
        self.forward_noise = float(new_f_noise*dt);
        self.turn_noise    = float(new_t_noise*dt);

        self.distance_noise = float(new_d_noise*dt);
    ...
    def set_seen(self,seen):
        self.seen = seen
    def set_real_measurements(radio_wheel,baselength):
        self.R = radio_wheel
        self.L = baselength
    def sense(self,node,i):
##        print node
        signature = node[2]

        Z = []
        # if self.orientation < 0:
        #     node[1] -= self.orientation
        # else:
        node[1] += self.orientation # measure angle
        node[1] = self.norm_ang(node[1])
        z_real = np.array([[node[0]],[node[1]]])
##        print z_real
        Z.append([i,z_real])
    ...
    def update_measurement(self,Z):
        ...
        for i,Z_ in enumerate(Z):
            ...
            z = Z_[1]
            # z[1][0] = z[1][0] + self.orientation
            ### from measurement get mean landmark, u,v
            r = z[0][0]
            theta = (z[1][0] + self.orientation)
            ...
            if Z_[0] not in self.seen: # new feature detected
                ...
                # Gz= [c -r*s;
                # s  r*c];
                H = np.array([[np.cos(theta),-r*np.sin(theta)],[np.sin(theta),r*np.cos(theta)]])
                self.P = np.linalg.inv(H).dot(Qt).dot(np.linalg.inv(H).T)

                param.append([Z_[0],self.l_mean,self.P])
                self.set_landmarks(param)
                self.seen.append(Z_[0]) # add signature

            else: # observed feature

                #update mean and cov measurement
                # print "self.landmark", self.landmarks
                ...
                idx_,l_mean,P_pred = self.landmarks[index] # mu_t, simga_t
                # print "l_mean",l_mean
                # print "x,y",self.x,self.y
                v_to_l_pred = [(l_mean[0][0]- self.x), (l_mean[1][0] - self.y)]
                # print "v_to_l_pred",v_to_l_pred
                landmark_angle_pred = np.math.atan2(v_to_l_pred[1],v_to_l_pred[0]) - self.orientation # measure angle
                ...
                self.P = new_sigma
                # compute weight
                weight = self.compute_weight(G,P_pred,Qt,residual,weight)

                for idx,n in enumerate([Z_[0],self.l_mean,self.P]):
                    self.landmarks[index][idx] = n
        self.weight *= weight

    def move(self, turn, forward, pos,particle=False):
        # if forward < 0:
        #     raise ValueError, 'Robot cant move backwards'
        xt_1 = [self.x,self.y,self.orientation]#[self.x,self.y,self.orientation]
        xt = [pos[0],pos[1],pos[2]]
        # print "xt",xt
        new_xt = sampling([xt_1,xt])
        # print "xt_1",xt_1
        # print "new_xt",new_xt
        ...
        # x = new_xt[0]
        # y = new_xt[1]
        # orientation = self.norm_ang(new_xt[2])
        # set particle
    ...
    def compute_weight(self,G,P_pred,Qt,residual,w):

        ### Weights FastSLAM 1.0 ###
        # print "G",G
        # print "P_pred",P_pred
        # print "Qt", Qt
        # print "residual",residual
        Zt = G.dot(P_pred).dot(G.T)+Qt #innovation covariance Gaussianly
        # print "Zt",Zt
        id_exp = ( (-.5*(residual.T)).dot(np.linalg.inv(Zt)).dot(residual))
        # print "id_exp",id_exp
        # print "np.math.exp",np.math.exp(id_exp)
        ...
        w *= round(np.math.exp(id_exp)/normalizer,6)
        return w
